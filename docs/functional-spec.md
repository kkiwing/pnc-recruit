...
- 단계 카드 목록: 이름(인라인 편집), 현재 상태값 뱃지 나열, 순서 이동(위/아래 버튼), "상태 관리" 버튼, 삭제 버튼, 아코디언 형태의 자동발송 설정. (단계에는 "구분" 셀렉트가 없다 — 단계별 합불 판정 개념 자체가 폐지됐기 때문에 2.3. "완료 입력폼" 셀렉트도 없다 — 날짜 입력 트리거가 단계가 아니라 상태 속성(`hasDateInput`)으로 옮겨갔기 때문에 2.4.)
- 하단 "단계 추가" 폼(이름만 입력 — 구분·완료 입력폼 지정 항목 없음).
- **상태 관리 모달**: 이름 입력, 색상 스와치, 상태 목록 맨 위 항목에 자동으로 붙는 "시작" 배지(순서를 바꾸면 따라감, 2.12), 각 상태마다 "단계종료"·"날짜+메모" 체크박스, 순서 이동(위/아래 버튼), 삭제 버튼. 그 외 항목은 없다 — "구분" 셀렉트와 "합격으로"/"불합격으로" 버튼은 2026-07-15 course-correction으로 제거됐다.

**사용자 액션과 결과**
- 셀렉트에서 공고 ↔ 기본 프리셋 전환 → 편집 대상만 바뀌고 URL의 `posting` 쿼리가 함께 갱신된다.
- 단계 이름 변경, 순서 이동, 삭제, 추가 → 프리셋 모드면 전역 프리셋 상태를, 공고 모드면 그 공고의 `stages`를 갱신한다(2.2 참고). 새로 추가하는 단계는 항상 대기·진행중·완료 기본 상태 세트(`progressStatuses()`)를 자동으로 갖고 시작한다 — `hasDateInput`은 꺼진 채로 시작하므로, 날짜 입력이 필요하면 상태 관리에서 직접 켜야 한다(2.4).
- "상태 관리" → 그 단계의 상태값 편집 모달. "단계종료" 체크 시 그 상태가 목록 맨 아래로 이동하고 같은 단계의 다른 모든 상태의 체크는 예외 없이 자동 해제된다(완전 배타, 2.12). "날짜+메모" 체크는 그 상태로 바뀔 때 날짜 입력 모달이 뜨게 한다.
...
- **목록형**: 지원자명(클릭 시 상세로 이동)·이메일, 공고/팀, "전형" 셀렉트(2.6에서 설명한 대로 보기 전용 선택), "상태" 셀렉트(`hasDateInput` 상태로 바꾸면 모달), **"최종 결과"**(미정이면 흐린 "미정" 버튼, 지정되면 합격=success/불합격=destructive 배지 + 메모 있으면 아이콘 — 클릭하면 항상 `FinalResultModal`이 열려 새로 지정하거나 고치거나 "판정 해제"로 되돌릴 수 있다, 2.11), 지원일(줄바꿈 방지), 메모 아이콘(메모 있으면 진하게·없으면 흐리게 표시하되 항상 노출, 클릭 시 메모 보기/작성 모달), "..." 메뉴(상세보기/별도관리 이동(사유 자유 입력 모달, 2.7)/삭제). `finalResult`가 지정된 행은 "전형"·"상태" 셀렉트와 날짜 재입력 버튼이 모두 잠긴다(2.13).
